...
# Dataset code https://github.com/CAMMA-public/cholect50/blob/master/dataloader_pth.py
# for each video
class T50(Dataset):
...
class CholecT50():
    ...
    def split_selector(self, case='cholect50'):
        switcher = {
            ...
            'cholect50-challenge': {
                'train': [1], #[1, 15, 26, 40, 52, 79, 2, 27, 43, 56, 66, 4, 22, 31, 47, 57, 68, 23, 35, 48, 60, 70, 13, 25, 49, 62, 75, 8, 12, 29, 50, 78, 6, 51, 10, 73, 14, 32, 80, 42],
                'val':   [1], # [5, 18, 36, 65, 74],
                'test':  [1] # [92, 96, 103, 110, 111]
            },
            'cholect45-crossval': {
    ...
    def transform(self):
        ...
        # normalize = transforms.Normalize(mean=data_config['mean'], std=data_config['std'])
        input_size = (3,224,224) # data_config.get('input_size', (3,224,224))
        C,H,W = input_size
        op_test   = [transforms.Resize((H,W)), transforms.ToTensor(), ]
        op_train  = [transforms.Resize((H,W))] + self.augmentation_list + [transforms.Resize((H,W)), transforms.ToTensor()]
        if self.normalize:
